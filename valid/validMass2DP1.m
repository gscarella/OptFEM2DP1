function validMass2DP1()
% function validMass2DP1()
%  Validation function for the assembly of the Mass matrix for
%  P1-Lagrange finite element method.
%
%   The Mass matrix (M) is computed by functions MassAssembling2DP1{Version} where {Version} is one of
%     {'base', 'OptV0', 'OptV1','OptV2'}.
%     - Test 1: Computation of the Mass Matrix using all the versions giving errors and computation times
%     - Test 2: Approximation of the integral of u.v on unit square by 
%                    dot(M*U,V)
%       where U=u(Th.q(1,:),Th.q(2,:))' and V=v(Th.q(1,:),Th.q(2,:))'.
%       Functions u and v are those defined in valid_FEMmatrices. 
%     - Test 3: Ones retrieves the order 2 of P1-Lagrange integration 
%        
% See also:
%   MassAssembling2DP1base, MassAssembling2DP1OptV0,
%   MassAssembling2DP1OptV1, MassAssembling2DP1OptV2,
...
%   see README for details
  ...
  disp('******************************************')
  disp('*     Mass Assembling P1 validations     *')
  disp('******************************************')
...
% TEST 1
  ...
  T(1)=toc();
  tic();
  MOptV0=MassAssembling2DP1OptV0(Th.nq,Th.nme,Th.me,Th.areas);
  T(2)=toc();
  Test1.error(1)=norm(Mbase-MOptV0,Inf);
  Test1.name{1}='MassAssembling2DP1OptV0';
  ...
  Test1.name{2}='MassAssembling2DP1OptV1';
  ...
  Test1.name{3}='MassAssembling2DP1OptV2';
...
% TEST 3
  ...
  for k=1:10  
    ...
    fprintf('      MassAssembling2DP1OptV2 CPU times : %3.3f(s)\n',TT(k));
    fprintf('      Error                           : %e\n',Error(k));
  end
